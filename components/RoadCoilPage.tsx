...
import { Truck, CheckCircle, ArrowRight, Cog, Layout, Thermometer, Maximize, Maximize2, Box, Globe, Shield, ShieldAlert, Anchor, Zap, Weight, X } from 'lucide-react';

interface RoadCoilPageProps {
...
const RoadCoilPage: React.FC<RoadCoilPageProps> = ({ onNavigate, onQuote, lang }) => {
  ...
  const services = lang === 'EN' ? [
    'Complete loads', 'Partial loads', 'Groupage loads', 'Speed freight services',
    'Harbour services', 'Custom formalities', 'Exceptional transport', 'Coiltrailers'
  ] : [
    'Complete ladingen', 'Deelladingen', 'Groupage ladingen', 'Speed freight diensten',
    'Havendiensten', 'Douaneformaliteiten', 'Uitzonderlijk transport', 'Coiltrailers'
  ];
  ...
  return (
    <div className="pt-24 min-h-screen bg-white">
      ...
      <div className="relative h-[45vh] min-h-[400px] flex items-center overflow-hidden bg-navy">
        <div className="absolute inset-0 z-0">
          <img
            ...
            onError={(e) => { e.currentTarget.src = "https://images.unsplash.com/photo-1519003722824-194d4455a60c?auto=format&fit=crop&q=80&w=2000"; }}
          />
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
          <div className="max-w-3xl">
            <h1 className="text-4xl md:text-6xl font-black text-white mb-6">
              {lang === 'EN' ? 'Road & Coil Transport' : 'Weg- & Coiltransport'} <br />
              <span className="text-safety-orange">{lang === 'EN' ? 'Across Europe.' : 'Door Heel Europa.'}</span>
            </h1>
            <p className="text-xl text-slate-300 font-light leading-relaxed">
              {lang === 'EN'
                ? 'Versatile fleet solutions for every cargo type, including specialized coiltrailers for the steel industry.'
                : 'Veelzijdige vlootoplossingen voor elk type lading, inclusief gespecialiseerde coiltrailers voor de staalindustrie.'}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="relative py-16 bg-white flex items-center justify-center">
        ...
        <div className="relative z-10 flex items-center gap-4 bg-white px-8">
          <Cog className="w-8 h-8 text-eu-yellow" />
          <span className="text-2xl font-black text-navy uppercase tracking-widest">{lang === 'EN' ? 'Coil Transport' : 'Coiltransport'}</span>
          <Cog className="w-8 h-8 text-eu-yellow" />
        </div>
      </div>

      {/* ── COIL HERO BANNER ── */}
      <div className="relative h-[50vh] min-h-[420px] flex items-center overflow-hidden bg-navy">
        <div className="absolute inset-0 z-0">
          <img src={COIL_HERO_BG} alt="Steel Industry" className="w-full h-full object-cover opacity-40 scale-105" />
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 w-full">
          <div className="max-w-3xl">
            ...
            <p className="text-xl text-slate-300 font-light leading-relaxed max-w-2xl border-l-4 border-eu-yellow pl-6">
              {lang === 'EN'
      ...
      <section className="py-24 bg-white">
        <div className="max-w-4xl mx-auto px-4 text-center">
          <div className="bg-white p-16 rounded-[4rem] shadow-2xl border border-slate-100 relative overflow-hidden">
            ...
            >
              {lang === 'EN' ? 'REQUEST A QUOTE' : 'VRAAG EEN OFFERTE AAN'}
            </button>
          </div>
        </div>
      </section>
